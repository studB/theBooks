// ============================================================
// theBooks — Top-level app (workspace / file list / editor)
// ============================================================
...
function App() {
  const [items, setItems] = useStateA(loadItems);
  const [workspaceId, setWorkspaceIdInner] = useStateA(() => {
    const stored = loadWorkspace();
    return stored && loadItems().some(i => i.id === stored) ? stored : null;
  });
  const [currentFolderId, setCurrentFolderId] = useStateA(workspaceId);
  const [openFileId, setOpenFileId] = useStateA(null);
  const [splitFileId, setSplitFileId] = useStateA(null);
  const [splitWidth, setSplitWidth] = useStateA(420);
  const [dialogOpen, setDialogOpen] = useStateA(false);
  ...
  useEffectA(() => { saveItems(items); }, [items]);

  // Breadcrumb of folders from workspace down to currentFolderId (inclusive)
  const breadcrumbForList = useMemoA(() => {
  ...
  // Clear split if the file no longer exists (deleted)
  useEffectA(() => {
    if (splitFileId && !splitFile) setSplitFileId(null);
  }, [splitFileId, splitFile]);

  const breadcrumbForEditor = useMemoA(() => {
    if (!openFile) return [];
    const stack = [];
  ...
  function updateOpenFile(patch) {
    if (!openFileId) return;
    setItems(its => its.map(it =>
      it.id === openFileId ? { ...it, ...patch, updatedAt: Date.now() } : it
    ));
  }
  ...
  return (
    <div className={`app ${openFile ? 'with-chat' : ''} ${splitFileId ? 'with-split' : ''}`} style={splitFileId ? { '--split-width': splitWidth + 'px' } : null}>
      {openFile ? (
        <>
          {splitFile && (
            <>
              <ReferencePane
                ...
                onSwap={() => {
                  // Swap: split becomes primary, primary becomes split
          ...
          <Editor
            ...
            items={items}
            workspaceId={workspaceId}
            breadcrumb={breadcrumbForEditor}
            onChange={updateOpenFile}
            onSaveNow={updateOpenFile}
            onExit={() => { setOpenFileId(null); setSplitFileId(null); }}
