// ============================================================
// theBooks — Native-style folder picker dialog (macOS Finder)
// ============================================================

const { useState: useStateD, useMemo: useMemoD, useRef: useRefD, useEffect: useEffectD } = React;

// Simulated filesystem prefix where theBooks data lives
const FS_PREFIX = ['Users', 'jay', 'Documents', 'theBooks'];
...
function pathFor(items, folderId) {
  // Returns array of names from theBooks root down to folderId
  const trail = [];
  let id = folderId;
  while (id) {
...
function FolderPickerDialog({ open, items, initialFolderId, onCancel, onSelect, onCreateAtRoot }) {
  const [browseId, setBrowseId] = useStateD(null); // current folder being viewed (null = theBooks root)
  const [selectedId, setSelectedId] = useStateD(initialFolderId || null);
  const [sidebarSelected, setSidebarSelected] = useStateD('documents');
...
window.FolderPickerDialog = FolderPickerDialog;
window.pathFor = pathFor;
window.FS_PREFIX = FS_PREFIX;
